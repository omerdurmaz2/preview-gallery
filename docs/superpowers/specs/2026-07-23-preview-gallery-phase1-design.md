...
| D4a | Repeated direct `@Preview` on one function | **One entry**, labelled by the **function name** (not by any single config's `name`) | `@Preview(name="Light") @Preview(name="Dark") fun ScreenPreview()` is common. v1 renders nothing, so a per-config split buys nothing; collapsing to one entry keeps discovery working, and using the function name avoids a label that names only one of several configs |
