...
| D11 | Index reads happen inside `ReadAction.compute`, on the HTTP handler's own thread. The `ProjectSnapshot` itself is **not** cached at this layer — only `PreviewIndexService.findAll`/`findOrphanSnapshots` are, and only inside that service, which depends on both `PsiModificationTracker.MODIFICATION_COUNT` and its own `refreshTracker`. | The handler thread is not the EDT and holds no read lock, so touching PSI without one is an outright violation. A `McpServerService`-level cache keyed on `MODIFICATION_COUNT` alone was tried (PG17-10) and reverted (PG17-11): later indexing passes fill the index without touching PSI, so a cached row set from the smart window between two such passes kept being served after `refreshTracker` had already moved past it — the same false negative D10 exists to prevent, reached through a cache instead of dumb mode. What actually makes the read action cheap is `lineOf`: it prefers `FileDocumentManager.getCachedDocument` over `getDocument`, which would load and decode a file just to answer one line lookup, and falls back to reading a file's raw text once per **call** (a `MutableMap` built fresh in `snapshot()` and discarded with it, so it can never go stale) rather than once per row. |
...
- **Claude Code** — `npx -y mcp-remote http://localhost:7891/mcp`
- **Cursor** — the same, in its `mcpServers` shape
- **Raw URL** — for anything that speaks Streamable HTTP directly

Codex is not a tab here (PG17-10 item 8): it reads TOML (`[mcp_servers.…]`), not the `mcpServers` JSON the other
two share, and this dialog does not ship a TOML snippet.
