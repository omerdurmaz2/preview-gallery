...
| D6 | **A non-empty search query auto-expands the matching branches** so results are visible without clicking; clearing the query returns to D5's module-level state. | Filtering already narrows the tree to matches — leaving them collapsed would hide the very rows the query selected. |
...
| D9 | Every branch row shows its subtree preview count, grayed, the way module rows already do. | With chains compacted, a branch label alone does not say how much is under it; the count is what makes a collapsed row informative. |
